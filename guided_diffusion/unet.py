...
class AntiCausalMechanism(nn.Module):
    def __init__(self, encoders: List[EncoderUNetModel], out_labels: List[str], out_channels: List[int],
        ...
        anticausal_predictors = []
        for out_label, out_channel, encoder in zip(out_labels, out_channels, encoders):
            anticausal_predictors.append(nn.Sequential(
                nn.Linear(encoder.out_channels, classifier_width),
                nn.ReLU(),
                nn.Linear(classifier_width, out_channel),
            ))
        self.anticausal_predictors = nn.ModuleList(anticausal_predictors)

    def forward(self, x, timesteps):
        out = {}
        latents = [enc(x, timesteps) for enc in self.encoders]
        for anticausal_predictor, out_label, latent in zip(self.anticausal_predictors, self.out_labels, latents):
            if out_label == "thickness":
                input_latent = th.add(*latents)
            elif out_label == "slant":
                input_latent = latent
            else:
                raise Exception
